...
namespace test01.Api.Endpoints.v1.Users
{
    ...
    {
        [Authorize]
        public static async Task<Results<Ok<DeleteUserResult>, BadRequest<ValidationResult>, NotFound, Conflict, ProblemHttpResult>>
            DoAsync(int id, [FromServices] IMessageBus bus, CancellationToken ct)
        {
            ArgumentNullException.ThrowIfNull(bus, nameof(bus));
            var result = await bus.InvokeAsync<CommandResult<DeleteUserResult>>(new DeleteUser(id), ct);
